<form method="POST" action="{{ $action }}">
    ...
    <!-- Candidate Name -->
    <div class="mb-6">
        <x-form.label for="name" :value="__('Name')" class="text-lg font-medium text-gray-800" />
        <x-form.input id="name" class="block mt-2 w-full" type="text" name="name" :value="old('name', $candidate->name ?? '')" required autofocus />
    </div>

    <!-- Candidate Party -->
    <div class="mb-6">
        <x-form.label for="party" :value="__('Party')" class="text-lg font-medium text-gray-800" />
    ...
    <div class="mb-6">
        ...
        <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach($elections as $election)
                <label class="block bg-gray-100 rounded-lg shadow p-4">
